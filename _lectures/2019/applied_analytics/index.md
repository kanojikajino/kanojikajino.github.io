...
- 提出場所: レタス「[最終レポート課題](https://letus.ed.tus.ac.jp/mod/assign/view.php?id=361788)」
...
- EM アルゴリズムの実装
  - 最低限欲しいもの
	- 内部状態 = モデルパラメタ
	- データを用いて内部状態を更新する命令: 例えば最尤推定する関数 `fit`
	- 内部状態を用いて何か計算する命令: 例えば対数尤度を計算する関数 `log_pdf`
  - あると便利なもの（例）
    - E-step を実行する関数 `e_step`
